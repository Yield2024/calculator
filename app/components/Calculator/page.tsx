...
export default function Calculator() {
  ...
  const generateDataPointsComp = () => {
    ...
    for (let i = 1; i <= termYears; i++) {
      ...
      const value = calcularInteresCompuestoReinversion(
        numericValue,
        i,
        tasaEfectiva
      );
  ...
  function CustomTooltip({
    ...
    payload?: Array<{
      payload: { title: string; value: number; value2: number };
    }>;
    label?: number;
    active?: boolean;
  }) {
    ...
    if (active && payload) {
      ...
      return (
        <div className={styles.custom_tooltip}>
          ...
          <p className="intro">{`Compounding Interest  - $${new Intl.NumberFormat(
            "en-US"
          ).format(value2 - numericValue)}`}</p>
          <p className="desc">{`Total - $${new Intl.NumberFormat(
            "en-US"
          ).format(value2)}`}</p>
          <p className="intro">{`Simple Interest  - $${new Intl.NumberFormat(
            "en-US"
      ...
      <div className={styles.main}>
        ...
        <div className={styles.main_right}>
          ...
          <div className={styles.main_right_stats_wrapper}>
            ...
            <div>
              <div style={{ display: "inline-block", marginRight: "20px", color: "#ffffff", }}>
                <div
                  style={{
                    width: "10px",
                    height: "10px",
                    backgroundColor: "#03feeb",
